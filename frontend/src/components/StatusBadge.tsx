...
export const StatusBadge: React.FC<{ status: string; size?: 'sm' | 'md' }> = ({ status, size = 'md' }) => {
  ...
  if (s.includes('resolve') || s.includes('complete') || s.includes('success')) {
    ...
    colorStyle = 'bg-orange-50 text-orange-700 border-orange-200/80 dark:bg-orange-500/10 dark:text-orange-400 dark:border-orange-500/20';
    dotColor = 'bg-orange-500';
  } else if (s.includes('cancel') || s.includes('escalat') || s.includes('critical') || s.includes('reopen')) {
  ...
  } else if (s.includes('review') || s.includes('submit') || s.includes('start') || s.includes('plan')) {
