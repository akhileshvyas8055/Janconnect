        ...
        <AppLayout>
            <div className="p-6 md:p-8 max-w-7xl mx-auto w-full space-y-7">
                ...
                {loadingHistory ? <LoadingCard text="Loading Civic Intelligence datasets…" /> : !dataset ? (
                    ...
                    {dataset.processingStatus === 'Analyzed' && <section className="space-y-3"><div><p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Civic Intelligence map</p><h2 className="font-bold text-slate-900 dark:text-white">Complaint reports and optional incident layer</h2></div><div className="h-[560px] rounded-xl overflow-hidden border border-slate-200 dark:border-white/10"><MapView civicClusters={filteredClusters} civicRecords={dataset.records} civicPriorityFilter={priorityFilter} initialCivicLayerVisible onViewIncident={setDetailCluster} /></div></section>}
                </>}
            </div>
